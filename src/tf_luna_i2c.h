...
class TFLuna {
    public:
        TFLuna(ros::NodeHandle& nh);
        TFLuna(const TFLuna&) = delete;
        TFLuna(TFLuna&&) = delete;
        
        bool readFromSensor();

        void publish();
        
        void spinOnce(const ros::TimerEvent&);
        ...
        bool readReg( uint8_t nmbr);
        bool writeReg( uint8_t nmbr, uint8_t data);
    ...
    private:
        i2c_ros::I2C m_tfl_control=i2c_ros::I2C(0, 0x10);
        double m_range;
        int16_t m_signal_strength;
        int16_t m_chip_temperate;

    private:
        ros::NodeHandle &m_nh;
        ros::Publisher m_range_publisher;

        std::string m_base_frame_id;
        std::string m_range_frame_id;
